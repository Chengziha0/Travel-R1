...
def compute_data_metrics(batch, use_critic=True):
    ...
    if 'tool_action_stats' in batch.meta_info:
        metrics['env/number_of_tool_action'] = float(np.array(batch.meta_info['tool_action_stats'], dtype=np.int16).mean())
        # if 'valid_action_stats' in batch.meta_info:
            # metrics['env/ratio_of_tool_action'] = float((np.array(batch.meta_info['tool_action_stats'], dtype=np.int16) / np.array(batch.meta_info['valid_action_stats'], dtype=np.int16)).mean())
    if 'memory_action_stats' in batch.meta_info:
        metrics['env/number_of_memory_action'] = float(np.array(batch.meta_info['memory_action_stats'], dtype=np.int16).mean())
        # if 'valid_action_stats' in batch.meta_info:
            # metrics['env/ratio_of_memory_action'] = float((np.array(batch.meta_info['memory_action_stats'], dtype=np.int16) / np.array(batch.meta_info['valid_action_stats'], dtype=np.int16)).mean())
    if 'answer_action_stats' in batch.meta_info:
        metrics['env/number_of_answer_action'] = float(np.array(batch.meta_info['answer_action_stats'], dtype=np.int16).mean())
        # if 'valid_action_stats' in batch.meta_info:
        #     metrics['env/ratio_of_answer_action'] = float((np.array(batch.meta_info['answer_action_stats'], dtype=np.int16) / np.array(batch.meta_info['valid_action_stats'], dtype=np.int16)).mean())
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    print(f"DEBUG: final_gen_batch_output: {final_gen_batch_output}")
